...
import kmods.colorpicker.ColorPickerView;

import static kmods.Utils.getResID;

public class RangoPrefs extends DialogPreference {
	...
	public RangoPrefs(Context context) {
		this(context, null);
	}
	public RangoPrefs(Context context, AttributeSet attrs) {
		super(context, attrs);
		summaryText = super.getSummary();
		if (attrs != null) {
			TypedArray a = context.getTheme().obtainStyledAttributes(attrs, new int[]{getResID("ColorPicker", "styleable")}, 0, 0);
			selectNoneButtonText = a.getString(getResID("ColorPicker_colorpicker_selectNoneButtonText", "attr"));
			noneSelectedSummaryText = a.getString(getResID("ColorPicker_colorpicker_noneSelectedSummaryText", "attr"));
			showAlpha = a.getBoolean(getResID("ColorPicker_colorpicker_showAlpha", "attr"), true);
			showHex = a.getBoolean(getResID("ColorPicker_colorpicker_showHex", "attr"), true);
		}
		else {
	...
	private View addThumbnail(View view) {
		...
		LayoutInflater.from(getContext()).inflate(
				isEnabled()
					? getResID("color_preference_thumbnail", "layout")
					: getResID("color_preference_thumbnail_disabled", "layout"),
				widgetFrameView);
		return widgetFrameView.findViewById(getResID("thumbnails", "id"));
	}
	...
	private void showColor(Integer color) {
		...
		if (thumbnail != null) {
			...
			thumbnail.findViewById(getResID("colorPreview", "id")).setBackgroundColor(thumbColor == null ? 0 : thumbColor);
		}
