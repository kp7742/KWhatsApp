...
public class ColorPickerView extends FrameLayout {
	...
	public ColorPickerView(Context context, AttributeSet attrs) {
		super(context, attrs);
		LayoutInflater.from(context).inflate(getResID("picker", "layout"), this);

		swatchView = (SwatchView)findViewById(getResID("swatchView", "id"));
		swatchView.observeColor(observableColor);

		HueSatView hueSatView = (HueSatView)findViewById(getResID("hueSatView", "id"));
		hueSatView.observeColor(observableColor);

		ValueView valueView = (ValueView)findViewById(getResID("valueView", "id"));
		valueView.observeColor(observableColor);

		alphaView = (AlphaView)findViewById(getResID("alphaView", "id"));
		alphaView.observeColor(observableColor);

		hexEdit = (EditText)findViewById(getResID("hexEdit", "id"));
	...
	private void applyAttributes(AttributeSet attrs) {
		if (attrs != null) {
			TypedArray a = getContext().getTheme().obtainStyledAttributes(attrs, new int[]{getResID("ColorPicker", "styleable")}, 0, 0);
