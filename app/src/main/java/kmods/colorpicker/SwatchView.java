...
public class SwatchView extends SquareView implements ColorObserver {
	...
	public SwatchView(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray a = context.getTheme().obtainStyledAttributes(attrs, new int[]{getResID("SwatchView", "styleable")}, 0, 0);
			radialMarginPx = a.getDimension(getResID("SwatchView_radialMargin", "attr"), 0f);
		}
		else {
			radialMarginPx = 0;
		}
